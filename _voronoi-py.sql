CREATE OR REPLACE FUNCTION py_voronoi(table_name text,geom_col text, trim_geom text) returns SETOF record as $$
...
# calculate extent
extent = {}
rv = plpy.execute("SELECT ST_SetSRID(ST_Extent(ST_Buffer(ST_Union(%s, (%s)), 0.01)), %i) as extent FROM %s" % (geom_col, trim_geom, srid, table_name), 1)
